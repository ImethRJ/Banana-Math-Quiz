...
let score = 0;
let lives = 3; // Medium level has 3 lives
let timeLeft = 35; // Medium level has 35 seconds
let timer;
...
function checkAnswer() {
    ...
    if (parseInt(userAnswer) === correctAnswer) {
        ...
        score += 25; // More points for medium level
...
function updateLives() {
    let hearts = "";
    for (let i = 0; i < 3; i++) { // Medium level has 3 hearts
        hearts += i < lives ? "❤️" : "💔";
    }
...
function resetTimer() {
    clearInterval(timer);
    timeLeft = 35; // Medium level has 35 seconds
...
function restartGame() {
    score = 0;
    lives = 3; // Reset to 3 lives for medium level
    scoreElement.innerText = score;
    updateLives();
...
function saveScore() {
    ...
    const scoreData = {
        ...
        lives: lives,
        timeLeft: timeLeft,
        difficulty: "medium" // Add difficulty level for filtering in leaderboards
    };
